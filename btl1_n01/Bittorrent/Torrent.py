...
class TorrentInfo:
    def __init__(self, torrent_file_path):
        ...
        info_bencoded = bencodepy.encode(info)
        # Request wil take take info_hash_url, as it's the correct format for info_hash
        self.info_hash = hashlib.sha1(info_bencoded).hexdigest() # used for internal mapping
        self.info_hash_raw = hashlib.sha1(info_bencoded).digest()
        self.info_hash_url = urllib.parse.quote_from_bytes(self.info_hash_raw, safe='') # used for tracker url
        
        # Checking if info_hash is valid
        assert len(self.info_hash_raw) == 20
        assert all(self.info_hash_url.startswith('%') or c.isalnum() for c in self.info_hash_url)
        decoded = urllib.parse.unquote_to_bytes(self.info_hash_url)
        assert decoded == self.info_hash_raw
        assert decoded.hex() == self.info_hash
        print("Torrent's info_hash should be in correct form!")
